...
function setUpDropDownMenu() {
    ...
    if(dropDownMenuIcon==null) return;
...
function setUpSlideRightMenu() {
    ...
    let projectForum = document.querySelector(
        "#slide-right-project-forum-link"
    );
    let notifications = document.querySelector(
        "#slide-right-project-notifications-link"
    );
    let settings = document.querySelector("#slide-right-project-settings-link");
    let mainOptionsContainer = document.querySelector(
        "#slide-right-menu-main-options"
    );
    ...
    projectIcons.forEach(function (item, index) {
        item.addEventListener("click", function () {
            ...
            notifications.addEventListener("click", function () {
                ...
                let notificationBalls = document.querySelectorAll(
                    ".project-icon-number-notifications-container"
                );
                ...
                sendAjaxRequest(
                    ...
                    function () {
                        ...
                        responseHandler();
                        let assignedTaskNotifications = JSON.parse(
                            this.response
                        )[0];
                        let taskCommentNotifications = JSON.parse(
                            this.response
                        )[1];
                        ...
                        assignedTaskNotifications.forEach((element) => {
                            let newAssignedTaskNotification =
                                document.createElement("div");
                            newAssignedTaskNotification.setAttribute(
                                "class",
                            ...
                            taskDescription.innerHTML = element[
                                "description"
                            ].substring(0, 30);
                        ...
                        taskCommentNotifications.forEach((element) => {
                            let newCommentNotification =
                                document.createElement("div");
                            newCommentNotification.setAttribute(
                                "class",
                                "notification assigned-notification"
                            );
                            let firstText = document.createElement("p");
                            ...
                            projectMembers.forEach((member) => {
                                if (
                                    element["project_member_id"] == member["id"]
                                ) {
                            ...
                            firstText.innerHTML =
                                commentedByName +
                                " commented on a task you are assigned to:";
